...
import AddWageComponent from './AddWageComponent';
...
const obj = {
    ...
    "data": [
        {
            "addTime": "2025-03-02T16:14:58.387Z",
            "id": 0,
            "month": "2025-03-02T16:14:58.387Z",
            "monthMoney": 0,
            "updateTime": "2025-03-02T16:14:58.387Z"
        }
    ],
    "lastPage": true,
    "pageSize": 0,
...
function Wage() {
    ...
    // 分页请求
    const wagePage = async () => {
        const res = await wageApi.wagePageApi(pageParams);
        setPageData(res);
    };

    // 编辑
    const editClick = async (id) => {
        const res = await wageApi.detailWageApi(id);
    ...
    const handleConfirmDelete = async () => {
        try {
            await wageApi.deleteWageApi(deleteId);
            message.success('月薪删除成功');
            wagePage();
        } catch (error) {
            message.error('月薪删除失败');
            console.error('删除月薪时出错：', error);
        }
    ...
    return (
        <>
            <AddWageComponent visible={visible} onCancel={onCancel} editData={editData} />
